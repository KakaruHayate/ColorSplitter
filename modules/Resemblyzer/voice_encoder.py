...
class VoiceEncoder(nn.Module):
    def __init__(self, device: Union[str, torch.device]=None, verbose=True, weights_fpath: Union[Path, str]=None):
        ...
        # Define the network
        self.lstm = nn.LSTM(mel_n_channels, model_hidden_size, model_num_layers, batch_first=True, bidirectional=True)
        self.dropout = nn.Dropout(0.5)
        self.linear = nn.Linear(model_hidden_size * 2, model_embedding_size)
        self.batch_norm = nn.BatchNorm1d(model_embedding_size)
        self.relu = nn.ReLU()
    ...
    def forward(self, mels: torch.FloatTensor):
        ...
        embeds_raw = self.relu(self.linear(hidden_concat))
